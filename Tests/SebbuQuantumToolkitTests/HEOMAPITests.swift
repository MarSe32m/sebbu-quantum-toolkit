...
struct HEOMAPITests {
    ...
    func heomZeroBathGKSL(shift: HEOM.ShiftType) throws {
        let h = TimeDependentOperator { t, matrix in
            matrix[0, 0] = 0
    ...
    func heomDynamicBath(shift: HEOM.ShiftType) throws {
        ...
        original.system.hamiltonian.insert(t: 0, into: &h0)
        free.system.hamiltonian.insert(t: 0, into: &hf)
        let full = Matrix(copying: h0)
        let uncoupled = Matrix(copying: hf)
        let h = TimeDependentOperator { t, out in
            for i in 0..<(4 * n * n) {
