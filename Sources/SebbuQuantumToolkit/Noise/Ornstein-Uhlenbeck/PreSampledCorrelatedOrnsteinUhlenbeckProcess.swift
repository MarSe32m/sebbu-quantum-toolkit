...
public struct PreSampledCorrelatedOrnsteinUhlenbeckProcess: ComplexNoiseProcess, Sendable {
    ...
    @inlinable
    public init<Generator: RandomNumberGenerator>(LC: Matrix<Complex<Double>>, LR: Matrix<Complex<Double>>, F: [Complex<Double>], start: Double, end: Double, step: Double, generator: inout Generator) {
        ...
        for i in 0..<xi.count { xi[i] = generator.nextNormal(stdev: .sqrt(0.5)) }
        LC._dot(xi, into: &x)
    ...
    @inlinable
    public init<Generator: RandomNumberGenerator>(r: [Complex<Double>], W: [Complex<Double>], start: Double, end: Double, step: Double, generator: inout Generator) {
        let (F, LC, LR) = constructFLCLR(r: r, W: W, dt: step)
        self.init(LC: LC, LR: LR, F: F, start: start, end: end, step: step, generator: &generator)
    }
...
public struct PreSampledCorrelatedOrnsteinUhlenbeckProcessGenerator: Sendable {
    @usableFromInline
    ...
    @inlinable
    @inline(always)
    public func generate<Generator: RandomNumberGenerator>(generator: inout Generator) -> PreSampledCorrelatedOrnsteinUhlenbeckProcess {
        PreSampledCorrelatedOrnsteinUhlenbeckProcess(LC: LC, LR: LR, F: F, start: start, end: end, step: step, generator: &generator)
    }
}

@inlinable
internal func constructFLCLR(r: [Complex<Double>], W: [Complex<Double>], dt: Double) -> (F: [Complex<Double>], LC: Matrix<Complex<Double>>, LR: Matrix<Complex<Double>>) {
