...
extension HOPS.CPUEngine: HOPS.MultiTimeOrderedCorrelationImplementation {
	...
	public func solveMultiTimeOrderedCorrelation<Hamiltonian>(
		...
		let threadCount = BLAS.getNumThreads()
		BLAS.setNumThreads(1)
		defer { BLAS.setNumThreads(threadCount) }
			...
			while trajectoryID < execution.trajectoryIDs.upperBound {
				...
				do {
					...
					return .init(
						...
						failure: .init(
							trajectoryID: trajectoryID, error: error)
					)
				}
				completed += 1
		...
		sums.withLock { values in
			...
			{
				if observer(outputTimes[index], inverseCount * values[index])
					== .stop
				{
					summary = .init(
