...
public struct ClosureHamiltonian: ~Copyable, Sendable, HamiltonianFunction {
	...
	@inlinable
	public init(
		_ closure: @Sendable @escaping (Double, inout UniqueMatrix<Complex<Double>>) -> Void
	) {
...
extension QuantumSystem where Hamiltonian == ClosureHamiltonian {
	@inlinable
	public init(
		dimension: Int,
		hamiltonian:
			@Sendable @escaping (Double, inout UniqueMatrix<Complex<Double>>) -> Void
	) {
